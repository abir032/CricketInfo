...
class CoreDataDB{
    ...
    func savePlayersData(data: [[String: Any]]){
        let batchInsert = NSBatchInsertRequest(entity: Player.entity(), objects: data)
        do {
            _ = try context.execute(batchInsert) as? NSBatchUpdateResult
        }catch {
            debugPrint(error)
        }
        }
    func getAllData()-> [Player]?{
            var searchPlayer = [Player]()
            let fetchRequest = NSFetchRequest<Player>(entityName: "Player")
            do{
                searchPlayer = try context.fetch(fetchRequest)
                return searchPlayer
            }catch{
                print(error)
                return nil
